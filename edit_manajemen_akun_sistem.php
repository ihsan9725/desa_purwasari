...
<body>
    ...
    <div id="page-content-wrapper">
      ...
      <div class="container-fluid">
        <h1 class="mt-4">Edit Manajemen Akun Sistem</h1> 
        <p>Desa Purwasari</p>
      </div>
          ...
          <div class="card card-primary">
            ...
            <div class="card-body">
               ...
               <?php
                include "koneksi.php";
                $id = $_GET['id'];
                $query_mysqli = mysqli_query($koneksi, "SELECT * FROM tb_auth WHERE id='$id'")or die(mysqli_error());
                while($d = mysqli_fetch_array($query_mysqli)){
                ?>
          
               <form action="update_manajemen_akun_sistem.php" method="post" enctype="multipart/form-data">
              <div class="form-group">
                       
                <input type="hidden" name="id" value="<?php echo $d['id']?>">
                <label for="inputName">Username</label>
                <input type="text" name="username" id="username" class="form-control" placeholder="Masukan Username" value="<?php echo $d['username']?>">
               
              </div>
              <div class="form-group">
                <label for="inputDescription">Password</label>
                <input type="text" name="pass" id="pass" class="form-control" rows="" placeholder="Masukkan Password" value="<?php echo $d['pass']?>">
              </div>
               <div class="form-group">
                <label for="inputDescription">Level</label>
                <select class="form-control" name="level" id="level">\
                  <option selected="selected"><?php echo $d['level']?></option>
                  <option>warga</option>
                  <option>admin</option>
                  <option>kades</option>
                </select>
                <!-- <input type="text" name="level" id="level" class="form-control" rows="" placeholder="Tambahkan Foto" value="<?php echo $d['level']?>"> -->
              </div>
